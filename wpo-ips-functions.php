...
/**
 * Conditionally converts an <img> tag with a local .avif source to use a .jpg fallback instead.
 *
 * If the <img> tag contains a `.avif` file, this checks if a `.jpg` version already exists.
 * If not, it attempts to convert the image using GD (if PHP ≥ 8.1) or Imagick.
 * Only applies to local paths or URLs that map to the local uploads folder.
 *
 * @param string $image_html The HTML string of the <img> element.
 * @return string The modified HTML with .jpg fallback if applicable, or the original HTML.
 */
function wpo_ips_maybe_convert_avif_img_tag( string $image_html ): string {
	if ( preg_match( '/src=["\'](.*?\.avif)["\']/', $image_html, $matches ) ) {
		$src      = $matches[1];
		...
		// Use existing JPG immediately, if available
		if ( WPO_WCPDF()->file_system->exists( $jpg_path ) ) {
			return preg_replace(
				'/src=["\']' . preg_quote( $src, '/' ) . '["\']/',
				'src="' . esc_attr( $jpg_src ) . '"',
				$image_html
			);
		}
		
		// Only try conversion if AVIF exists and JPG does not
		if ( WPO_WCPDF()->file_system->exists( $src_path ) ) {
			try {
				if (
					version_compare( PHP_VERSION, '8.1.0', '>=' ) &&
					extension_loaded( 'gd' ) &&
					is_callable( 'imagecreatefromavif' )
				) {
					$image = imagecreatefromavif( $src_path );
					if ( $image !== false ) {
						imagejpeg( $image, $jpg_path, 90 );
						imagedestroy( $image );
					}
				...
				wcpdf_log_error( 'Error converting AVIF image to JPG', 'error', $e );
				return $image_html;
			}
		}
		
		// After attempted conversion, use it if it now exists
		if ( WPO_WCPDF()->file_system->exists( $jpg_path ) ) {
			return preg_replace(
				'/src=["\']' . preg_quote( $src, '/' ) . '["\']/',
				'src="' . esc_attr( $jpg_src ) . '"',
