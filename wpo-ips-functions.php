 ...
 */
function wpo_ips_replace_avif_images( string $html ): string {
	//todo just return HTML in case a new advanced setting is disabled
	...
	if ( ! empty( $html ) ) {
		...
		if ( ! empty( $replaced_images ) ) {
			...
			foreach ( $replaced_images as $avif_src => $jpg_src ) {
				...
				if ( ! WPO_WCPDF()->file_system->exists( $avif_src ) ) {
					continue;
				}

				// Skip if jpg already exists
				if ( WPO_WCPDF()->file_system->exists( $jpg_src ) ) {
					continue;
				}
