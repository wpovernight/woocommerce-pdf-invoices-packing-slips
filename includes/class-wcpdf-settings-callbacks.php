...
class Settings_Callbacks {
	...
	public function next_number_edit( $args ) {
		extract( $args );
		$next_number  = $store->get_next(); // $store is a 'Sequential_Number_Store' object
		$nonce        = wp_create_nonce( "wpo_wcpdf_next_{$store->store_name}" );
		printf( '<input id="next_%1$s" class="next-number-input" type="text" size="%2$s" value="%3$s" disabled="disabled" data-store="%1$s" data-nonce="%4$s"/> <span class="edit-next-number dashicons dashicons-edit"></span><span class="save-next-number button secondary" style="display:none;">%5$s</span>', $store->store_name, $size, $next_number, $nonce, __( 'Save', 'woocommerce-pdf-invoices-packing-slips' ) );
		// Displays option description.
		if ( isset( $description ) ) {
			printf( '<p class="description">%s</p>', $description );
		}
