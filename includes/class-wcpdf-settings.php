...
class Settings {
	...
	/**
	 * Get the user role settings capability.
	 * @return string
	 */
	public function user_settings_capability( $use_default = false ) {
		$default_capability  = apply_filters( 'wpo_wcpdf_settings_default_user_role_capability', 'manage_woocommerce' );	
		$custom_capabilities = apply_filters( 'wpo_wcpdf_settings_custom_user_role_capabilities', [] );
		$user_capability     = '';
		...
		if ( empty( $user_capability ) || $use_default ) {
			$user_capability = $default_capability;
		}
