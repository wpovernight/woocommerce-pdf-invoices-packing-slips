			...
			if ( ! empty( $_POST['search'] ) && ! empty( $_POST['document_type'] ) ) {
				...
				if ( ! empty( $results ) ) {
					...
					foreach ( $results as $value ) {
						...
						$data[$order_id]['billing_first_name'] = is_callable( array( $order, 'get_billing_first_name' ) ) ? WPO_WCPDF()->sanitizer->sanitize_html( $order->get_billing_first_name() ) : '';
						$data[$order_id]['billing_last_name']  = is_callable( array( $order, 'get_billing_last_name' ) ) ? WPO_WCPDF()->sanitizer->sanitize_html( $order->get_billing_last_name() ) : '';
						$data[$order_id]['billing_company']    = is_callable( array( $order, 'get_billing_company' ) ) ? WPO_WCPDF()->sanitizer->sanitize_html( $order->get_billing_company() ) : '';
