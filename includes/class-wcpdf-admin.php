	...
	public function save_invoice_number_date( $order_id, $order ) {
		...
		if ( $order_type == 'shop_order' ) {
			...
			if ( $invoice = wcpdf_get_invoice( $order ) ) {
				...
				if  ( $invoice->get_date() && ! $invoice->get_number() && is_callable( array( $invoice, 'add_number' ) ) ) {
					$invoice->add_number();
				}
		...
		try {
			...
			if( ! empty( $document ) ) {
				...
				// on save
					...
					// check if we have number, and if not generate one
					if( $document->get_date() && ! $document->get_number() && is_callable( array( $document, 'add_number' ) ) ) {
						$document->add_number();
					}
