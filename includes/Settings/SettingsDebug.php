...
class SettingsDebug {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_before_delete_order', array( $this, 'on_delete_order' ), 10, 2 );
		add_action( 'before_delete_post', array( $this, 'on_delete_post' ), 10, 2 );
		
		if ( false === get_option( 'wpo_ips_deleted_orders_table_created', false ) ) {
			$this->create_deleted_orders_table();
		}
	}
	...
	public function fetch_number_table_data( string $table_name, string $orderby = 'id', string $order = 'desc', string $from = '', string $to = '', int $chunk_size = 100, int $offset = 0 ): void {
		...
		// Also get deleted rows in the same date range
		$deleted_rows = $this->get_deleted_numbers_for_table(
			$table_name,
			$data['from'],
			$data['to'],
			$orderby,
		...
		if ( ! empty( $deleted_rows ) ) {
			$chunk_results = array_merge( $chunk_results, $deleted_rows );
		}
	...
	/**
	 * Create the deleted orders table if it doesn't exist.
	 *
	 * @return void
	 */
	private function create_deleted_orders_table(): void {
		global $wpdb;
		$table   = $wpdb->prefix . 'wpo_ips_deleted_orders';
		$collate = $wpdb->get_charset_collate();
		
$sql = "CREATE TABLE $table (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  order_id BIGINT UNSIGNED NOT NULL,
  document_slug VARCHAR(50) NULL,
  document_number BIGINT UNSIGNED NULL,
  document_date DATETIME NULL,
  deleted_at DATETIME NOT NULL,
  deleted_by BIGINT UNSIGNED NULL,
  PRIMARY KEY  (id),
  KEY order_id (order_id),
  KEY doc_lookup (document_slug, document_number),
  KEY document_date_idx (document_date),
  KEY deleted_at_idx (deleted_at)
		...
		update_option( 'wpo_ips_deleted_orders_table_created', true );
	}
	
	/**
	 * Log a deleted order in the deleted orders table.
	 *
	 * @param \WC_Order|\WP_Post $order The deleted order object.
	 * @return void
	 */
	private function log_deleted_order( object $order ): void {
		static $seen = array();
		...
		$documents = WPO_WCPDF()->documents->get_documents();
		foreach ( $documents as $document ) {
			$number_data    = $order->get_meta( "_wcpdf_{$document->slug}_number_data", true ); // Note: `wcpdf_get_document()` cannot retrieve documents for orders that are in the trash.
			$date_timestamp = $order->get_meta( "_wcpdf_{$document->slug}_date", true );
			
			$doc_number = ( isset( $number_data['number'] ) && is_numeric( $number_data['number'] ) )
			...
			$wpdb->insert(
				$wpdb->prefix . 'wpo_ips_deleted_orders',
				array(
					'order_id'        => (int) $order_id,
					'document_slug'   => (string) $document->slug,
					'document_number' => $doc_number,
					'document_date'   => $date_timestamp ? wp_date( 'Y-m-d H:i:s', (int) $date_timestamp ) : null,
	...
	/**
	 * Action hook callback for when an order is deleted.
	 *
	 * @param int       $order_id The ID of the deleted order.
	 * @param \WC_Order $order    The deleted order object.
	 * @return void
	 */
	public function on_delete_order( int $order_id, \WC_Order $order ): void {
		$this->log_deleted_order( $order );
	}

	/**
	 * Action hook callback for when a post is deleted.
	 *
	...
	/**
	 * Return deleted rows (stdClass) compatible with number tables.
	 *
	 * Shape:
	 *   - id                => document_number
	 *   - order_id          => order_id
	 *   - date              => document_date (fallback to deleted_at if requested)
	 *   - calculated_number => NULL
	 *
	 * @param string $table_name Full number table name (e.g. "{$wpdb->prefix}wcpdf_invoice_number")
	 * @param string $from       MySQL datetime (inclusive)
	 * @param string $to         MySQL datetime (inclusive)
	 * @param string $orderby    One of: id|date|order_id|calculated_number
	 * @param string $order_dir  ASC|DESC
	 * @param bool   $fallback_to_deleted_at Use deleted_at when document_date is NULL
	 * @return array<stdClass>
	 */
	private function get_deleted_numbers_for_table( string $table_name, string $from, string $to, string $orderby = 'id', string $order_dir = 'DESC', bool $fallback_to_deleted_at = true ): array {
		global $wpdb;

		$doc_slug = $this->get_doc_slug_from_number_table( $table_name );
		if ( ! $doc_slug ) {
			return array();
		}

		$allowed_orderby = array( 'id', 'date', 'order_id', 'calculated_number' );
		...
		return $wpdb->get_results( $deleted_query ); // stdClass rows
	}
