jQuery(function(a){function b(a){a&&(h.hide().filter(`[data-tax-class="${a}"]`).show(),g.removeClass("active").attr("aria-pressed","false").filter(`[data-tax-class="${a}"]`).addClass("active").attr("aria-pressed","true"))}function c(){const b=a(".edi-tax-class-select").val();a.get(wpo_ips_edi.ajaxurl,{action:"wpo_ips_edi_reload_tax_table",nonce:wpo_ips_edi.nonce,tax_class:b},function(c){const d=a(`.edi-tax-class-table[data-tax-class="${b}"]`);d.html(c)})}function d(){const b=a("#edi-customer-order-id"),c=b.val(),d=b.closest("table"),f=d.find("tbody");if(!c)return f.empty().append(`<tr><td colspan="2">${wpo_ips_edi.enter_order_id}</td></tr>`),!1;const g=a.trim(c);return isNaN(g)||""===g?(f.empty().append(`<tr><td colspan="2">${wpo_ips_edi.valid_number}</td></tr>`),!1):(f.empty().append(`<tr><td colspan="2">${wpo_ips_edi.loading}</td></tr>`),a.get(wpo_ips_edi.ajaxurl,{action:"wpo_ips_edi_load_customer_order_identifiers",nonce:wpo_ips_edi.nonce,order_id:c},function(b){if(f.empty(),b.success&&b.data&&b.data.data&&0<Object.keys(b.data.data).length){const c=b.data.data;a.each(c,function(a,b){let c=b.label||a,d=b.value,g="",h="";("undefined"==typeof d||null===d||""===d)&&(g=b.required?"#d63638":"#996800",d=`<span style="color:${g};">${b.required?wpo_ips_edi.missing:wpo_ips_edi.optional}</span>`),"vat_number"===a&&b.value&&!e(b.value)&&(h=`<br><small style="color:#996800;">${wpo_ips_edi.vat_warning}</small>`),f.append(`
						<tr>
							<td>${c}</td>
							<td>${d}${h}</td>
						</tr>
