			...
			<td colspan="3">
				<?php
					printf(
						...
						'<code>' . esc_url( wp_normalize_path( WPO_WCPDF()->main->get_tmp_path() ) ) . '</code>',
						'<code>' . esc_url( wp_normalize_path( $upload_base ) ) . '</code>',
						'<code>UPLOADS</code>',
			...
			<td colspan="3">
				<?php
					printf(
						...
						'<code>' . esc_url( wp_normalize_path( WPO_WCPDF()->plugin_path() . '/vendor/dompdf/dompdf/lib/fonts/' ) ) . '</code>'
					);
				?>
			</td>
		</tr>
